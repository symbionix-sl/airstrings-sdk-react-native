...
describe('CanonicalJSON', () => {
  ...
  it('encodes astral-plane characters as 4-byte UTF-8', () => {
    const bundle: StringBundle = {
      format_version: 1,
      project_id: 'proj_test12345678',
      locale: 'en',
      ...
      key_id: 'key_test_01',
      signature: 'dummy',
      strings: { emoji: { value: 'done 🎉', format: 'text' } },
    }

    const result = signedContent(bundle)
    expect(result).toEqual(new TextEncoder().encode(toUTF8(result)))
    expect(toUTF8(result)).toContain('done 🎉')
  })
