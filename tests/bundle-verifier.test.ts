...
function makeSignedBundle(
  ...
  const publicKey = overrides.publicKey ?? ed.getPublicKey(overrides.privateKey)
  ...
  const signatureBytes = ed.sign(canonicalBytes, overrides.privateKey)
...
describe('BundleVerifier', () => {
  it('accepts a valid signature', async () => {
    ...
    const bundle = makeSignedBundle({ privateKey, publicKey })
  ...
  it('rejects wrong key', async () => {
    ...
    const wrongPublic = ed.getPublicKey(wrongKey)
  ...
  it('rejects unknown key_id', async () => {
    ...
    const otherPublic = ed.getPublicKey(otherKey)
  ...
  it('rejects unsupported format_version', async () => {
    ...
    const bundle = makeSignedBundle({ privateKey, publicKey, format_version: 99 })
  ...
  it('rejects tampered strings', async () => {
    ...
    const original = makeSignedBundle({
      privateKey,
      publicKey,
